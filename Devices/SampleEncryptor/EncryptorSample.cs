    ...
    public class EncryptorSample : ICryptoDevice, IKeyManagementDevice, ICommonDevice
    {
                                                     ...
                                                     CancellationToken cancellation)
        {
            ...
            if (request.Algorithm == "T")
            {
                ...
                if (request.VerifyAttribute is not null &&
                    (request.VerifyAttribute.VerifyMethod == ImportKeyRequest.VerifyAttributeClass.VerifyMethodEnum.KCVZero ||
                     request.VerifyAttribute.VerifyMethod == ImportKeyRequest.VerifyAttributeClass.VerifyMethodEnum.KCVSelf))
                {
                    if (request.VerifyAttribute.VerifyMethod == ImportKeyRequest.VerifyAttributeClass.VerifyMethodEnum.KCVZero)
                    {
                        keyCheckValue = new(request.KeyData.Count);
                        for (int i = 0; i < keyCheckValue.Count; i++)
                            keyCheckValue[i] = 0;
                    }
                    else
                    {
                        checkKey = tDESEncrypt.Key;
                    }
                }
                ...
                if (checkKey is not null)
                {
                    ...
                    verifyAttrib = new ImportKeyResult.VerifyAttributeClass("00", "T", "V", request.VerifyAttribute.VerifyMethod, request.VerifyAttribute.HashAlgorithm);
            ...
            {
                // PKCS #1.5 (Signature) - request.VerifyAttribute.VerifyMethod.CryptoMethod == KeyManagementCapabilitiesClass.VerifyMethodClass.VerifyMethodEnum.RSASSA_PKCS1_V1_5
                // PKCS #7 (TR34) - request.VerifyAttribute.VerifyMethod.CryptoMethod == KeyManagementCapabilitiesClass.VerifyMethodClass.VerifyMethodEnum.RSASSA_PSS
            }
        ...
        public async Task RunAsync()
        {
            for (; ; )
            {
                ...
                await cryptoServiceProvider.InitializedEvent(new InitializedEvent.PayloadData());
            }
        }
