...
public class ClientConnection(Uri EndPoint)
{
    ...
    public async Task<object> ReceiveMessageAsync(CancellationToken CancelTaken = default)
    {
        try
        {
            var buffer = new byte[InitialMessageBufferSize];

            // Get the next message. The buffer starts at InitialMessageBufferSize and doubles
            // whenever it fills up before EndOfMessage is reached, so messages larger than the
            // initial size (e.g. a Camera.TakePicture completion carrying a multi-megabyte
            // base64-encoded image) are still read in full instead of being silently truncated.
            ValueWebSocketReceiveResult rc;
            int ReceivedBufferReceived = 0;
            do
            {
                if (ReceivedBufferReceived == buffer.Length)
                {
                    Array.Resize(ref buffer, buffer.Length * 2);
                }

                var BufferSlice = new Memory<byte>(buffer, ReceivedBufferReceived, buffer.Length - ReceivedBufferReceived);
                // Wait for data from the client
                rc = await Socket.ReceiveAsync(BufferSlice, CancelTaken);
                ReceivedBufferReceived += rc.Count;
            } while (!rc.EndOfMessage);


            if (rc.MessageType == WebSocketMessageType.Text)
            {
                // trim the incomming message and extract a string
                var messageString = Encoding.UTF8.GetString(buffer, 0, ReceivedBufferReceived);
    ...
    /// <summary>
    /// Starting buffer size for receiving incoming messages; grows (doubling) if a message
    /// turns out to be larger than this.
    /// </summary>
    private const int InitialMessageBufferSize = 2 * 1024 * 1024; // 2MB

    /// <summary>
    /// Endpoint of this device to connect
    /// </summary>
    private readonly Uri EndPoint = EndPoint ?? throw new ArgumentException("Endpoint required");
